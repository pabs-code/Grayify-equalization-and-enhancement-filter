...
class ImageGrayscaleConverter:
    ...
    def show_original_image(self):
        if self.original_image:
            st.image(self.original_image, caption="Original Image", use_container_width=True)
            self.thumbnail = cv2.cvtColor(np.array(self.original_image), cv2.COLOR_RGB2BGR)
    ...
    def show_grayscale_image(self):
        if self.grayscale_image:
            st.image(self.grayscale_image, caption="Grayscale Image", use_container_width=True)
    ...
    def apply_filter(self, filter_type):
        if self.grayscale_image:
            ...
            if img_eq is not None:  # Check if the image was processed
                st.image(img_eq, caption=f"Processed Image ({filter_type})", use_container_width=True)
            else:
...
def main():
    ...
    if uploaded_file:
        ...
        if converter.load_image():
            st.image(converter.original_image, caption="Original Image", use_container_width=True)
            ...
            filter_type = st.selectbox("Select a processing method", ["None", "Equalize", "Enhance"])
            if filter_type != "None":
